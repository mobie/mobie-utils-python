...
def _replace_name_in_data(storage_type, rel_path, new_name):
    if "bdv" in storage_type:
        bdv_metadata.write_name(rel_path, setup_id=0, name=new_name)
    elif "ome.zarr" in storage_type:
        attrs_path = os.path.join(rel_path, ".zattrs")
...
def _replace_name_in_view(view, old_name, new_name):
    ...
    for display in displays:
        ...
        display["sources"] = update_sources(display["sources"])
        new_displays.append({display_type: display})
...
def rename_source(dataset_folder, old_name, new_name):
    ...
    if source_type in ("image", "segmentation"):
        ...
        for storage_type, rel_path in storage.items():
            ...
            _replace_name_in_data(storage_type, rel_path, new_name)
    ...
    for view_name, view in views.items():
        view = _replace_name_in_view(view, old_name, view_name)
