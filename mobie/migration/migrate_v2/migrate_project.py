...
from .intermediate.migrate_grid_spec2 import migrate_grid_spec2
...
def _update_grid_spec(root, ds_list):
    for ds in ds_list:
        ds_folder = os.path.join(root, ds)
        assert os.path.exists(ds_folder), ds_folder
...
def _update_grid_spec2(root, ds_list):
    for ds in ds_list:
        ds_folder = os.path.join(root, ds)
        assert os.path.exists(ds_folder), ds_folder
        migrate_grid_spec2(ds_folder)
...
def migrate_project(root, parse_menu_name=None, parse_source_name=None,
                    ...
                    update_grid_spec2=False):
    assert sum((update_view_spec, update_data_spec, update_table_spec, update_grid_spec)) <= 1
    already_v2 = update_view_spec or update_data_spec or update_table_spec or update_grid_spec or update_grid_spec2
    ...
    elif update_grid_spec2:
        _update_grid_spec2(root, ds_list)
    else:
